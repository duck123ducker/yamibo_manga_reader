    ...
    if(mode==='scrollView'){
        return (
            <View ref={imageRef} onLayout={handleLayout} style={[styles.loading, style]}>
                {loaded ? <MangaCoverImage visible={visible} author={author} height={style.height} width={style.width} id={id}/> : <View style={style}/>}
            </View>
        );
    }
    if(mode==='flatList'){
...
export default React.memo(MangaCoverImageLazyLoad);
